// Bounds on the paths an unauthenticated caller can drive: KV writes chosen by
// the caller, request bodies, batch fan-out, and KV reads per query.
...
describe("namespace-miss capture", () => {
  it("records a plausible unregistered domain", async () => {
    const feedback = countingKv(env.secid_FEEDBACK!);
    await resolveFromKV(env.secid_REGISTRY!, "secid:entity/some-new-vendor.io", {
      feedbackKv: feedback.kv,
    });
    expect(feedback.stats.puts).toEqual(["miss:entity/some-new-vendor.io"]);
  });

  it("does not record a namespace under a TLD that does not exist", async () => {
    const feedback = countingKv(env.secid_FEEDBACK!);
    const result = await resolveFromKV(env.secid_REGISTRY!, "secid:entity/foo.notarealtld", {
      feedbackKv: feedback.kv,
    });
    expect(result.status).toBe("not_found");
